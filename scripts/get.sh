...
# and container LOGS, because /var/lib/docker/containers is mode 0700 owned by
# root and no group grant opens it, leaving the Engine API as the only route to
# container stdout that does not mean running the whole agent as root.
#
...
# privileged container bind-mounting /. The agent only ever issues GETs, but the
# grant is not scoped to that, and quietly widening a host's privilege boundary
# during an unattended `curl | sudo bash` is not something to do on the
# operator's behalf.
...
if [[ ! -f /etc/agent-i/agent.yaml ]]; then
  ...
  # An existing config is never rewritten, so container collection is whatever
  # that file already says. Saying so matters: an operator who just granted
  # docker access on an upgrade would otherwise wait for container telemetry
  # that the config never asked for.
  if [[ "$INSTALL_DOCKER_ACCESS" == "1" ]] && ! grep -qE '^containers:' /etc/agent-i/agent.yaml; then
    echo "    it has no 'containers:' block — add one to collect container telemetry"
  fi
fi
...
# Say what container collection will and will not do on this host. Without this
# the failure is silent in the worst way: container metrics arrive, so the
# feature looks like it is working, while names show as short ids and no
